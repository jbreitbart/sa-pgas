...
static void multiply_matrix(const T& A, const T& B, T& C, const int a, const int b) {
	typename T::value_type * restrict Cptr  = C.get_tile_unitialized(a,b);
	...
	for (int t=0; t<SIZE/TILE_SIZE; ++t) {
		const typename T::value_type * restrict Aptr = A.get_tile(a,t);
		const typename T::value_type * restrict Bptr = B.get_tile(t,b);
...
static void compare(T1& B2, T2& B_seq, const int a, const int b) {
	const typename T1::value_type * restrict Cptr = B2.get_tile(a,b);
	const typename T2::value_type * restrict C_seqptr = B_seq.get_tile(a,b);
